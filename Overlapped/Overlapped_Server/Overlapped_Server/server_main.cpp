...
//	sprintf_s(clients[client_s].messageBuffer + dataBytes, MAX_BUFFER, "::From Server::");
//	clients[client_s].dataBuffer.len = static_cast<int>(strlen(clients[client_s].messageBuffer));
//
	...
	DWORD lnFlags)
{
	...
	// WSASend(응답에 대한)의 콜백일 경우
	SOCKETINFO& SI = clients[client_s];
	cout << "TRACE - Send message : " << clients[client_s].messageBuffer << " (" << dataBytes << " bytes)\n";
