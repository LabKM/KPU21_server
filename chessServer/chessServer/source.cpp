...
int main()
{
	...
	while (true) {
		...
		chess[clientSocket] = ChessPiece{};
		chess[clientSocket].x = 0;
		chess[clientSocket].y = 0;
...
void CALLBACK recv_callback(DWORD Error, DWORD dataBytes, LPWSAOVERLAPPED overlapped, DWORD lnFlags)
{
	...
	char* temp_buffer = new char[dataBytes];
	std::stringstream sstream;
	sstream.str(clients[client_s].messageBuffer);
	...
	if (type == 0) {
		...
		sprintf_s(clients[client_s].messageBuffer, "%d %d %d\0", 11, chess[client_s].x, chess[client_s].y);
	}
	else {
		sprintf_s(clients[client_s].messageBuffer, "UNKNOW VALUE");
	}
		
	clients[client_s].dataBuffer.len = strlen(clients[client_s].messageBuffer) + 1;
